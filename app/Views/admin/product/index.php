<div class="card-box">
    <h4 class="header-title"><?= $sub['product']['title']['index'] ?></h4>
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="javascript: void(0);">Dashboard</a></li>
        <li class="breadcrumb-item active"><?= $sub['product']['title']['index'] ?></li>
    </ol>
        ...
        <div class="ul right-0 position">
            <li>
                ...
                <ul class="dropdown-menu">
                    <li>
                        <a class="publishAll" data-field="product" data-column="publish" data-value="2"
                            href="#">Xuất bản</a>
                    </li>
                    <li>
                        <a class="publishAll" data-field="product" data-column="publish" data-value="1"
                            href="#">Huỷ
                            xuất bản</a>
                    </li>
                </ul>
    ...
    <form action="" method="GET">
        <div class="filter-box">
            <a class="btn btn-info" href="tao-san-pham"><?= $sub['product']['title']['create'] ?> </a>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <?php foreach ($val['catalogues'] as $catalogue) {?>
                        <a href="sua-danh-muc-code/<?=$catalogue['id']?>">- <?=$catalogue['name']?></a>   <br> 
                    <?php }?>
                    </td>
                    <td width="150px"><?= $val['canonical']  ?></td>
                    ...
                    <th width="50">
                        <input type="checkbox" <?= $val['publish'] == 1 ? 'checked' : '' ?> data-plugin="switchery"
                            ...
                            class="changeStatusPublish location-<?= $val['id'] ?>" data-filed="product"
                    ...
                    ?>
                    <th width="50px">
                        <a style="font-size: 20px;" href="sua-san-pham/<?= $val['id'] ?>"><i class="fe-edit"></i> </a>
                        <a style="font-size: 20px; color:rgb(241, 55, 55);" href="xoa-san-pham/<?= $val['id'] ?>"><i
                                class="fe-trash"></i></a>
                    </th>
                    <?php } else {?>
                    <th width="50px">
                        <a style="font-size: 20px;" title="Đưa trở lại" href="hoan-tac-san-pham/<?= $val['id'] ?>">
                            <i class="fe-rotate-ccw"></i>
                        </a>
                        <a style="font-size: 20px; color:rgb(241, 55, 55);" title="Xoá vĩnh viễn"
                            href="xoa-san-pham-vinh-vien/<?= $val['id'] ?>">
                            <i class="fe-trash"></i>
                        </a>
...
<ul class="pagination">
    <li class="page-item <?= $currentPage == 1 ? 'disabled' : '' ?>">
        <a class="page-link" href="san-pham?page=<?= $currentPage - 1 ?>&keyword=<?=$data['keyword']?>&status=<?=$data['publish']?>&flag=<?=$data['flag']?>" aria-label="Previous">
    ...
    <li class="page-item <?= $i == $currentPage ? 'active' : '' ?>">
        <a class="page-link" href="san-pham?page=<?= $i ?>&keyword=<?=$data['keyword']?>&status=<?=$data['publish']?>&flag=<?=$data['flag']?>"><?= $i ?></a>
    </li>
    <?php endfor; ?>

        <a class="page-link" href="san-pham?page=<?= $currentPage + 1 ?>&keyword=<?=$data['keyword']?>&status=<?=$data['publish']?>&flag=<?=$data['flag']?>" aria-label="Next">
            <span aria-hidden="true">»</span>
